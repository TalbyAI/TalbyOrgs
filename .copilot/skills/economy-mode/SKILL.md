...
|-------------|-------------|--------------|
| Writing code (implementation, refactoring, bug fixes) | `claude-sonnet-4.6` | `gpt-5.4-mini` |
| Writing prompts or agent designs | `claude-sonnet-4.6` | `gpt-5.4-mini` |
| Docs, planning, triage, changelogs, mechanical ops | `gpt-5.4-mini` | `gpt-5.4-mini` |
| Architecture, code review, security audits | `gpt-5.4` | `claude-sonnet-4.6` |
| Scribe / logger / mechanical file ops | `gpt-5.4-mini` | `gpt-5.4-mini` |

**Prefer `gpt-5.4-mini`** when the goal is to reduce cost for lightweight or exploratory work. Escalate back to `claude-sonnet-4.6` when the task still needs stronger code reliability or architectural judgment.
...
- **Don't override Layer 0 in economy mode.** If the user set `defaultModel: "gpt-5.4"`, they want quality. Economy mode only affects Layer 3 auto-selection.
...
- **Don't bump premium tasks down too far.** Architecture and security reviews shift from `gpt-5.4` to `claude-sonnet-4.6` in economy mode — they do NOT go to fast/cheap models.
