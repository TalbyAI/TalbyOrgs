...
**Layer 1 — Session Directive:** Did the user specify a model for this session? ("use gpt-5.4 for this session", "save costs"). If yes, use that model. Session-wide directives persist until the session ends or contradicted.
...
|-------------|-------|------|------|
| Writing code (implementation, refactoring, test code, bug fixes) | `claude-sonnet-4.6` | Standard | Balanced coding quality and reliability make Sonnet 4.6 the default for code. |
...
| NOT writing code (docs, planning, triage, logs, changelogs, mechanical ops) | `gpt-5.4-mini` | Fast | Cost first. Use the cheap agentic model for non-code tasks. |
| Visual/design work requiring image analysis | `gemini-3.1-pro-preview` | Specialist | Multimodal reasoning required. Prefer the visual specialist over a generic premium model. |
...
| Prompt Engineer | auto (per-task) | Mixed: prompt design is like code, research is not | Prompt architecture → `claude-sonnet-4.6`; research/analysis → `gpt-5.4-mini` |
| Copilot SDK Expert | `claude-sonnet-4.6` | Technical analysis that often touches code | Pure research → `gpt-5.4-mini` |
...
- **Bump UP to deep reasoning:** architecture proposals, reviewer gates, security audits, multi-agent coordination (output feeds 3+ agents) → `gpt-5.4`
- **Bump DOWN to fast/cheap:** typo fixes, renames, boilerplate, scaffolding, changelogs, version bumps
- **Switch to deep implementation support (`gpt-5.4`):** large multi-file refactors, complex implementation from spec, heavy code generation (500+ lines)
...
```
Premium:  gpt-5.4 → gemini-3.1-pro-preview → claude-sonnet-4.6 → (omit model param)
Standard: claude-sonnet-4.6 → gpt-5.4 → gpt-5.4-mini → (omit model param)
Fast:     gpt-5.4-mini → (omit model param)
```
...
Only set `model` when it differs from the platform default. If the resolved model matches the platform default, you MAY omit the `model` parameter — the platform uses it as default.
...
Premium: `gpt-5.4`, `gemini-3.1-pro-preview`
